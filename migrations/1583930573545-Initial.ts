...
export class Initial1583930573545 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE "${schema}"."user" ("userId" integer NOT NULL, "name" text, "walletAddress" text NOT NULL, "telegramUid" bigint, CONSTRAINT "user_pkey" PRIMARY KEY ("userId"))`, undefined);
        await queryRunner.query(`CREATE UNIQUE INDEX "user_telegramUid_idx" ON "${schema}"."user" ("telegramUid") `, undefined);
        await queryRunner.query(`CREATE TABLE "${schema}"."token" ("tokenId" integer NOT NULL, "name" text, "contractAddress" text NOT NULL, "symbol" text NOT NULL, "issuerUserId" integer, CONSTRAINT "token_pkey" PRIMARY KEY ("tokenId"))`, undefined);
        await queryRunner.query(`CREATE UNIQUE INDEX "token_symbol_idx" ON "${schema}"."token" ("symbol") `, undefined);
        await queryRunner.query(`ALTER TABLE "${schema}"."token" ADD CONSTRAINT "token_issuerUserId_fkey" FOREIGN KEY ("issuerUserId") REFERENCES "${schema}"."user"("userId") ON DELETE NO ACTION ON UPDATE NO ACTION`, undefined);
    }
